...
/// Parses the URL for a pending payment and extracts the relevant information.
/// Returns a [Pending] object if the URL contains pending data; otherwise, returns null.
Pending? pendingFromUrl(String urlString) {
  if (urlString.startsWith(Constants.pending)) {
    final Uri url = Uri.parse(urlString);
    final Map<String, String> queryParams = url.queryParameters;
    final String paymentKey = queryParams['paymentKey'] ?? '';
    final String orderId = queryParams['orderId'] ?? '';
    ...
    return Pending(paymentKey, orderId, amount);
  }
  return null;
}
